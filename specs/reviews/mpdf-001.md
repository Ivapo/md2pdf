...
written and once under `template: press-release`, page 1 of each. The images are not
committed; they were handed to the author in the session and are regenerated by the same
command. Read against the clause: the open and ticked boxes are told apart at a glance in
both looks; the sixty-word task hangs under its own first line, not under the box; no item
shows a bullet beside a box; a checklist nested in a plain item and a plain list nested in a
task item each sit where their parent puts them. The first render set the first line one
space right of the wrapped lines: the body's opening `[\n  ` becomes a space after the
separator. A weak `h` as the separator absorbs it, and both looks now say so in a comment.

**Gate (2)'s dump is recorded, not asserted.** The suite reads no PDF text, on the precedent
`the_author_date_fixture_compiles_to_a_pdf` set, so `pdftotext` was run by hand in both looks
(every body present, no `[ ]`, `[x]` or `[X]`; the one `]` is the escape item's own) and the
reading is written into the doc comment of
`core/tests/golden_test.rs:the_task_list_fixture_compiles_to_a_pdf_in_both_looks`.

**The mixed refusal is raised at `End(Item)`, not `End(List)`.** The phase says `End(List)`
picks among three arms, "anything else is the refusal". Raised at the end of the first item
whose kind differs, it carries the same message and the same line with one `Vec`, and it
fires in document order: an error inside a later item cannot be reported ahead of it.
`End(List)` keeps the two remaining arms.
