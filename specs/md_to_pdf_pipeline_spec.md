...
- **Scope:** In `core/src/emit.rs`: set `Options::ENABLE_FOOTNOTES`, and map
  ...
  **WIDENED 2026-08-18, by `mpdf-005` Phase 4: the exception is still one check
  ...
  > **CORRECTED 2026-09-25, by Phase 16.** The paragraph above is kept as it was
  > written. Its premise was never measured, and it is false: pulldown-cmark 0.13.4
  > never delivers a `FootnoteReference` inside an `Image`. A `[^1]` inside `![…]`
  > claims its own `]` first, and the image never forms, so the arm it kept
  > refused nothing. Phase 16 retires it.
...
*Produces the observable: yes — a PDF from a document that today does not compile at all,
each task item set under a box its look draws, empty or ticked as the source wrote it.*

**Drafted 2026-09-25**, and appended per §6.1 step 2. Phase 8 made a task list marker a named
error and said in its own scope that *"support is a later phase or a later spec; the named
error is the honest floor meanwhile"*. This is that phase.

- **Step 0 — a decision, not only code?** Yes. What the dialect accepts is a decision this
  spec owns, and so is the call contract the looks meet, which this phase widens by one
  export.
- **Step 1 — does it remove or contradict shipped work?** **It lifts a refusal Phase 8 named as
  provisional, and nothing built is un-built.** Phase 8's reason for refusing still holds and
  is what shapes this phase: Typst has no checkbox element, and a drawn marker is a look
  decision that §2 gives to the template and not to the emitter. So the emitter names the
  construct and the look draws it, as `divider` already does. No shipped prose is actively
  misleading. Phase 8 says *"later"*, and this is later, so no `CORRECTED` note is owed.
- **Step 2 — the subject.** The dialect and the looks' contract, which this spec has owned
  since Phase 1 and Phase 9. So a phase, not a spec.

- **Scope: the marker replaces the bullet, and the whole list crosses as one call.**

  **Why one call per list and not one per item.** Wrapping the marker in something like
  `- #task(checked: true) body` would be the smallest diff, but it sets a box *beside* a
  bullet. GitHub hides the bullet of a task item, and Pandoc's LaTeX writer puts the box
  where the bullet was (`\item[$\square$]`). Typst's `list` cannot do either one item at a
  time: `marker` is set per list and chosen by depth, and `list.item` takes no marker. What
  can put a box where the bullet was is a function that sees the whole list. So a task list
  crosses as a single call and the look lays it out. The look's own choice decides both the
  box and the hanging indent under it. `terms`, with the box as the term, is one shape that
  gives both, and it is named here as evidence that the call can be met, not as a
  requirement.

  **What crosses.** In `core/src/emit.rs`, a bullet list whose *every* item carries a marker
  is written as

  ```typst
  #checklist(tight: true, (checked: false, body: [an unchecked task]), (checked: true, body: [a checked task]))
  ```

  `tight` is `ListFrame`'s `loose`, negated, so the look can decide the spacing the plain list
  arm already expresses through its separator. `checked` is the `bool`
  `Event::TaskListMarker` carries. `body` is the item's translated content in a `[…]` block,
  laid out as the block-quote arm lays out its own: `[\n`, the content through
  `prefixed("  ", …)`, then `\n]`. The golden pins those bytes. **No new escaping is
  involved**: characters are escaped in the text arm, as they already are for every item,
  and the markup escape is what keeps a `]` in a body from closing its block. The item arm
  already calls `escaped_frame`, which is the group-frame check and not an escape, and that
  call stays where it is. A nested list inside a task item therefore arrives already
  rendered in its body, and a checklist nested inside a plain item is ordinary item content.
  Both directions of nesting are free.

  **The marker is recorded, not written.** `ListFrame` gains one `Vec<Option<bool>>`, parallel
  to `items`. The marker arm writes the innermost open list's current slot, and
  `End(TagEnd::Item)` pushes `None` when no marker arrived. **The marker arrives right after
  `Start(Item)` in a tight item, and inside the item's `Paragraph`, before its text, in a
  loose one.** That ordering was measured against pulldown-cmark 0.13.4 in round 1, and the
  gate's loose fixture is what fails if a later parser moves it. **`End(TagEnd::Item)`
  stops prefixing an item whose slot is `Some`**: it stores the trimmed content unprefixed,
  since the `- ` it writes today would land inside the body. `End(TagEnd::List)` then picks
  one of three arms: every slot `None` is today's arm, byte for byte; every slot `Some`
  writes `#checklist(…)`; anything else is the refusal below.

  **Two new refusals, built where they stand**, on the caption and link refusals' pattern
  rather than through `describe`:

  - **`task list marker in an ordered list`**, at the marker's line. The number and the box
    both claim the one place in front of the item. Choosing between them is exactly the
    guess §2's *"escapes and rejects, never guesses"* forbids, and GFM does not settle it:
    GitHub keeps both.
  - **`list mixing task items and plain items`**, at the line of the **first item whose
    kind differs from the list's first item**. The box replaces the bullet, so a mixed list
    would ask the look to set one list two ways. The fix is one keystroke either way, and
    the message's location points at it.

  **`describe` loses its `TaskListMarker` arm into the "supported construct" group**, since
  the walk now handles the event. The property Phase 8 established still holds: every arm
  `describe` still names is reachable. After this phase, `describe` names raw HTML and a
  footnote reference and nothing else, and its doc comment already says the right thing.

  > **CORRECTED 2026-09-25, by Phase 16.** The paragraph above is kept as it was written.
  > Its claim that every name `describe` keeps is reachable is false for the footnote
  > reference: pulldown-cmark 0.13.4 never delivers one inside an image, so no caller can
  > hand it to `describe`. Phase 16 retires that name.

  **The import is conditional**, on the `has_diagram` shape in `core/src/emit.rs:header`:
  the header grows a `has_checklist` flag, and `checklist` joins the import list only on a
  document that wrote one. This is what keeps every shipped golden file byte-identical, as
  the comment above that list already argues.

  **The flag also crosses back from a footnote definition, which is where the `has_diagram`
  shape alone would break.** Definitions are walked separately, in
  `core/src/emit.rs:collect_definitions`, and a list inside one is supported today. A diagram
  needs no path back because it is refused inside a definition. A checklist is not refused
  there, so a flag set only by the main walk would leave `#checklist(` on the page with no
  import, and Typst's unknown-variable error names neither the construct nor the line. **The
  flag takes `math`'s path**: `Body` gains a `checklist: bool` beside `math`, and the
  footnote-reference arm ORs it in exactly where it does `*math |= body.math`. An uncited
  definition sets nothing, which is right, since an uncited definition is never written.
  Refusing a checklist inside a footnote was the alternative, and it is set aside because a
  plain list is accepted there. Accepting one kind of list and refusing the other would be
  a line in the dialect drawn by the implementation.

  **Both looks export `checklist`**, in `core/assets/template.typ` and
  `core/assets/press-release.typ`. The contract goes **from five exports to six, and the
  call stays at nine**. **The box is drawn rather than typed as a glyph**, e.g. a stroked
  `box` a little under the cap height, with a tick path when `checked`. This is the one
  look choice the spec does make, and it follows from §2's bundled fonts: `U+2610`/`U+2611`
  exist only if a bundled face carries them, and a missing glyph compiles silently into
  tofu. Size, stroke, tick shape and indent are each look's own. The press release takes
  the same shape as the article unless its implementer finds a reason not to, and records
  that reason in a comment.

  **Untouched:** `core/src/frontmatter.rs`, `core/src/sections.rs`, the math and diagram
  modules and `cli/src`. The lists that `:::` groups and the front-matter blocks already
  refuse are refused before the marker is seen, so neither guard moves.

- **Exit gate:** seven cases.

  **The fixture is written out**, on Phase 10's rule that a second person reproduces a probe
  rather than inventing it. `tests/fixtures/task_list.md` holds, in order, **with a one-line
  paragraph between each list and the next, and a blank line either side of that
  paragraph**, so no two lists merge and no paragraph is lazily continued into the item
  above it:

  - a `# Task list` heading;
  - a tight checklist `- [ ] an unchecked task` / `- [x] a checked task`;
  - a loose checklist of two items, one `[X]` (capital, which GFM also accepts);
  - a bullet list whose second item nests a two-item checklist;
  - a checklist whose first item nests a plain two-item bullet list;
  - a one-item checklist whose body carries `#`, `]` and `*emph*`, for the escape;
  - a one-item checklist whose body is **one sentence of at least sixty words**, so that it
    wraps at least twice in the article's two-column measure and in the press release's
    wider one. This is the item gate (3)'s hanging-indent clause is read off.

  **The footnote case is a fixture of its own**, `tests/fixtures/task_list_footnote.md`: a
  `# Footnote` heading, a paragraph carrying `[^1]`, and the definition
  `[^1]: - [ ] a task in a footnote`. **No checklist stands outside the definition**, and
  that is the point. Round 2 caught that inside the main fixture the case passed for the
  wrong reason, because the six checklists in the main walk import `checklist` whether or
  not the flag crosses back.

  The existing `tests/fixtures/unsupported_task_list.md` is **renamed** to
  `tests/fixtures/mixed_task_list.md`, and a plain third item `- a plain item` is appended
  **on a line of its own**. The file ends today with no newline after `- [x] a checked task`,
  so one is written first. It is now the mixed refusal's fixture, and **the line it pins
  moves from 3 to 5**: the task items stand on lines 3–4 and the plain item on 5.

  (1) **The golden.** `tests/golden/task_list.typ` is the fixture's `md_to_typst` output,
  asserted in `core/tests/golden_test.rs` beside `LIST_SPACING_MD`, on its shape. It shows
  `checklist` in the import line and exactly **six** `#checklist(` calls, with
  `tight: false` on the loose one only. **Beside it, the footnote fixture gets an inline
  assertion rather than a golden**, on the shape Phase 12 gate (1) used. Its `md_to_typst`
  import line names `checklist`, its one `#checklist(` stands inside `#footnote[`, and its
  `md_to_pdf` starts with `%PDF`. This is the case that fails an implementation whose flag
  does not cross back from a definition, and it fails there twice: the import lacks the
  name, and the compile stops on an unknown variable.

  (2) **It compiles in both looks, and the brackets are gone.** `md_to_pdf` over the fixture
  starts with `%PDF`, once as written and once with `template: press-release` in a
  frontmatter block, the substitution Phase 11 gate (5) spelled out. A `pdftotext` dump of
  each carries every task body's text **and no `[ ]`, `[x]` or `[X]` anywhere**. That is
  the mechanical form of "the marker did not reach the page as text".

  (3) **The box is seen, once.** One `pdftoppm -r 150` page image per look is attached to
  the review record. Unchecked and checked are distinguishable at a glance, a wrapped task
  body hangs under its own first line rather than under the box, and no item shows a bullet
  beside a box. **This is the gate's one non-mechanical clause, and it is non-mechanical on
  purpose.** A drawn box is vector content that `pdftotext` cannot see, and asserting its
  path operators would pin a look choice this phase leaves to the look.

  (4) **Both refusals fire and name their line**, as rows of
  `core/tests/golden_test.rs:each_refused_construct_names_itself_and_its_line`. That test
  keeps its name and loses its only row. **It asserts `line == 3` for every row today, so
  each row gains its own expected line**, as a fourth tuple element. The mixed fixture
  refuses with `list mixing task items and plain items` at line 5, and an inline ordered
  case (`1. [ ] a`) refuses with `task list marker in an ordered list` at line 1. The test's
  doc comment, which narrates math leaving and *"the marker is what is left here"*, is
  rewritten. `cli/tests/cli_test.rs:math_and_a_task_list_marker_exit_non_zero_and_name_themselves`
  has its task row moved to the renamed fixture, the new construct name and `line 5`. It
  asserts `line 3` for both rows today, so it too gains a per-row line: the math row keeps
  3. **Its name stays**: the fixture still carries a marker, and the test exits non-zero on it.

  (5) **The contract grows by one needle, and a second one says the look reads the flag.**
  `core/tests/golden_test.rs:every_bundled_template_meets_the_call_contract` gains
  `#let checklist(` among the export needles, in the `#let X(` form its five neighbours
  take, so that a comment cannot satisfy it. A second needle, `checked`, rests on the
  argument the `affiliation` pair makes: a look that takes the item and ignores whether it
  is ticked satisfies the first needle alone. **`checked` is knowingly weaker than
  `super(`**. It names a field the look reads, not a call that renders it, and no call is
  common to every way of drawing a tick. Round 1 raised this and it was accepted as a
  stated limit. The rules paragraph's count, *"the export list is five
  and the call is nine"*, becomes six and nine.

  (6) **The front door's refused row becomes an accepted one.**
  `core/tests/examples_test.rs` checks the claims the dialect's public page made. Among
  them, `tests/fixtures/examples/error/task-list.md` is `REFUSALS`' row pinning
  `unsupported markdown construct 'task list marker' at line 1`. This phase makes that claim
  false, so **the fixture moves to `tests/fixtures/examples/ok/task-list.md` with its bytes
  unchanged**, and `REFUSALS` loses the row. The array type goes from `[_; 3]` to
  `[_; 2]`. `the_fixtures_are_the_pages_twelve_rows_and_its_two_assets` asserts ten
  accepted rows and two refusals, and its messages and doc comment say so. The row count
  stays twelve, so the test's name and the file's "twelve claims" survive. The module
  comment's *"a task list is refused"* is the one example of a public promise that
  becomes false, and it is replaced with one that is still true. **"Frozen" meant the page's
  bytes, not the page's verdicts.** The file tests a promise as a reader meets it, and a
  promise the engine no longer keeps is not one a test should keep asserting. Moving the
  row is the honest edit, and the bytes are untouched.

  (7) **Nothing else shipped moves, and the corpus stays clean.** `cargo test --workspace`
  passes with **no shipped golden edited**. That is the conditional import's whole
  argument, and the diff is where it shows. `spec-lint` exits with zero errors. The diff
  names `core/src/emit.rs`, the two looks, `core/tests/golden_test.rs`,
  `core/tests/examples_test.rs`, `cli/tests/cli_test.rs`, the two new fixtures and the golden, the
  renamed fixture, the moved example, the close-out's prose, this spec and the two
  regenerated indices, and nothing else.

- **Close-out.**

  `rules/pipeline.md`, four passages. **The dialect paragraph**: *"Twenty-seven things are
  supported"* becomes twenty-eight, and task lists join the list beside bullet lists.
  **A sentence under it** says a task list crosses as one `checklist` call and why, and
  names the two refusals. **The rejection paragraph at "Everything else is an error"**
  drops *"and a task list marker"*. **The paragraph after it**, which argues `describe`'s
  reachability *through* the marker, is rewritten around the two arms that remain. **The
  templates paragraph**: the export list and its count, the conditional import's
  enumeration, and the needle sentence. **The file is at 1405 of `max_lines: 1405`** as
  `spec-lint` prints it today, so **the cap moves with the edit**, as Phases 13 and 14 moved
  it. The new number is the measured count after the edit, plus nothing.

  **`core/src/emit.rs`, two comments**, in a file the phase already edits: the one in
  `options` saying *"only the task list marker still names itself as an error"*, which
  becomes false; and `describe`'s, which is checked and left as it is if it still reads
  true.

  `README.md`, two sites. *"A task list item, `- [ ] a`, is an error for now."* becomes a
  sentence saying task lists are supported, that the box replaces the bullet, and naming
  both refusals. **The examples paragraph near the top** (lines 31–34 as round 2 measured
  them) also changes: it says the page shows *"the three refusals"* and that
  `examples_test.rs` *"compiles the nine it accepts and holds the three it refuses"*. The
  second becomes ten and two with gate (6). The first describes Letur's live page, which
  keeps its three refusals until Letur takes the version, so it is reworded to stop counting
  rather than to count what the page does not yet show.

  **Samples: none needed.** `samples/showcase/showcase.md` is where a reader would look, but
  its anchors test pins heading lines. Adding a section there is a separate edit with its
  own trap (Phase 12's close-out), and a demo is not what this phase owes.

  **`CLAUDE.md`: none needed.** The observable sentence does not move. **Status artifact:
  none needed**, since `.spec-lint.yaml` has `status_artifacts: {}`. **Letur: none in this
  repository, and one debt named there.** The looks ship inside `core`, so Letur gains task
  lists by depending on the version that carries them. Publishing that version is not this
  phase. **Letur's `web/index.html` lists a task list among its refusals** (at `:556` and
  `:569` as round 1 measured it), and `mpdf-011`'s gate quotes that sentence. So Letur's page
  claim goes false the day Letur takes the version. That edit belongs to Letur's
  repository, at its bump, and this phase's report says so rather than leaving it to be
  discovered.

  **Commit plan: two commits, one push.** First, `feat(mpdf-001)`: the code, the looks, the
  fixtures, the golden and both test files. Second, `spec(mpdf-001)`: this phase's `shipped`
  date, the `rules/pipeline.md` passages and cap, the two README sites, and both indexes
  regenerated by `spec-lint --write-index`. The rollup goes `done` → `partial` as this phase
  is appended, and back to `done` when the second commit lands.


### Phase 16 — the footnote arm the reject path never sees

*Produces the observable: no. No input compiles differently, and none is refused
differently, before or after this phase. That is the point of it: the phase corrects what
the code and the rules claim about the reject path, and changes nothing the reader of a
PDF can see. It is argued for below rather than folded silently into a commit, because the
claim it corrects is Phase 8's stated property and Phase 7's stated decision.*

**Drafted 2026-09-25**, from a probe run while Phase 15 closed out.

- **Step 0 — a decision, not only code?** Genuinely unclear, so the cheapest spec action the
  test allows. The code change is one arm. But Phase 7 *decided* to keep that arm, for a
  reason written into this spec, and Phase 8 set the property the arm now breaks: *"every
  arm in `describe` is reachable … the property a later reader can check in one sitting"*.
  A commit alone would leave both statements standing.
- **Step 1 — does it remove or contradict shipped work?** It contradicts one shipped
  *premise*, not shipped behaviour. Phase 7's paragraph is actively misleading, so it gets
  a dated `CORRECTED` note in place, beside the text, and the original is kept. Nothing
  built is un-built: every input that compiles today compiles to the same bytes, and every
  refusal keeps its message.
- **Step 2 — the subject.** The dialect's reject path, which this spec has owned since
  Phase 1. So a phase.

**The evidence.** `core/src/emit.rs:describe` has two callers. The main walk's reject arm
never passes it a `FootnoteReference`, because the walk handles every one. The alt-text
capture's reject arm is the other caller, and it would pass one only if the parser
delivered a `FootnoteReference` between `Start(Tag::Image)` and `End(TagEnd::Image)`.
Twenty-five spellings were probed against pulldown-cmark 0.13.4 with this file's own
`options()`, each followed by a blank line and `[^1]: n`. **Each produced at least one
`FootnoteReference`, none of them inside an `Image`, and none formed an image at all.** The
`[^1]` claims its own closing bracket, and the `![` above it is left with no bracket to
close. The list, written out on Phase 10's rule that a second person reproduces a probe
rather than inventing it, as Rust string literals:

```text
"![a [^1] b](x.png)"            "![see[^1]](x.png)"             "![a[^1]](x.png)"
"![a [^1]b](x.png)"             "![[^1]](x.png)"                "![a[^1] ](x.png)"
"![ [^1] ](x.png)"              "![a[^1]][r]\n\n[r]: x.png"     "![a [^1]][r]\n\n[r]: x.png"
"![a[^1]]\n\n[a[^1]]: x.png"    "![a [^1]][]\n\n[a [^1]]: x.png" "![a [^1]]: x.png"
"![a *[^1]*](x.png)"            "![a `x` [^1]](x.png)"          "![a [^1][] b](x.png)"
"![a [b [^1] c](y) d](x.png)"   "![a ![b[^1]](y.png) c](x.png)" "![a\n[^1]](x.png)"
"![a [^1]](<x.png>)"            "![a [^1]] (x.png)"             "![a [^1]](x.png \"t\")"
"![a [^1]](x.png)[^1]"          "- ![a [^1]](x.png)"            "> ![a[^1]](x.png)"
"| h |\n|---|\n| ![a[^1]](x.png) |"
```

A twenty-sixth, `"![a\\[^1]](x.png)"`, was dropped from the list because its escaped bracket
leaves no reference at all, so it tests nothing. A reference *inside* inline code, as in
`` ![`[^1]`](x.png) ``, does form an image, but the `[^1]` is code text there and no
reference arrives, which is why no spelling of that shape is listed. Round 1's reviewer
ran forty-five spellings of their own, adversarial ones included, and none nested a
reference either.

- **Scope.** In `core/src/emit.rs`:

  - `describe` loses its `Event::FootnoteReference(_)` arm into the "supported construct"
    group, beside `TaskListMarker`. After this phase it names raw HTML — `raw HTML block`
    and `raw HTML` — and nothing else. Its doc comment, *"Every arm below is reachable"*,
    becomes true again **read as the rules read it, by construct name**, and is left as
    written. The name `raw HTML block` sits on both a `Start` and an `End` arm, and only
    the `Start` one can fire, since the walk stops there. That was true before this phase
    and is not what the property counts. Both raw HTML names stay reachable:
    `raw HTML` inside alt text is already pinned by
    `core/tests/golden_test.rs:an_out_of_dialect_construct_inside_alt_text_is_still_an_error`.
  - **The parse is pinned, not assumed.** A unit test in `emit.rs`'s `tests` module walks
    `parser(md)` for each of the twenty-five spellings above, each followed by
    `\n\n[^1]: n\n`. It asserts two things per spelling: that at least one
    `FootnoteReference` arrives, so no row passes by testing nothing, and that none arrives
    while an `Image` is open. It asserts **nesting**, not that
    no image forms: a later pulldown-cmark that forms an image *and* keeps the reference as
    text would be harmless, and failing on it would be noise. If an upgrade ever nests one,
    this test fails and names the spelling. It fails before the alt capture meets the event
    and would refuse it as `supported construct`, the nonsense message the alt-capture
    comment already warns about.

  **Untouched:** every other arm of the walk, both looks, every fixture and golden file,
  `cli/src`. The alt capture's own reject arm stays as it is. It still refuses raw HTML,
  and it is the path the pinned test guards.

- **Exit gate:** four cases.

  (1) **The pin.** The new unit test carries the twenty-five spellings listed above,
  verbatim, and passes. It fails on a parser that nests a reference, and it also fails on a
  row that yields no reference. Both assertions are on the event sequence, not on the
  emitted markup.

  (2) **Nothing moves.** `cargo test --workspace` passes with no fixture and no golden file
  edited, and `an_out_of_dialect_construct_inside_alt_text_is_still_an_error` still passes
  unedited. That is what shows the one remaining alt-text refusal survived the change.

  (3) **The claim is checkable in one sitting again.** The string literals in `describe`'s
  body are exactly `raw HTML block`, `raw HTML`, `markdown construct` and
  `supported construct`, and `FootnoteReference` appears in it only in the "supported
  construct" group. `grep -n '"footnote reference' core/src/emit.rs`, with the opening
  quote only, finds one line: the in-place refusal
  `"footnote reference inside a footnote definition"`. Today it finds that line and
  `describe`'s arm.

  (4) **The corpus stays clean.** `spec-lint` exits with zero errors. **The diff read is the
  build's**, from the commit that lands this phase's reviewed text to the phase's push. It
  names `core/src/emit.rs`, this spec (the `shipped` date alone), `rules/pipeline.md` and
  the two regenerated indices, and nothing else. The phase text, both `CORRECTED` notes and
  the review record land before that, in the review-round commit.

- **Close-out.**

  `rules/pipeline.md`, one passage: the paragraph after *"Everything else is an error"* says
  that two arms remain, *"raw HTML and a footnote reference"*. It becomes one arm, raw
  HTML, with one sentence on why a footnote reference cannot reach the alt capture, and the
  name of the test that pins it. The cap moves to the measured count if the edit changes it.

  **README: none needed.** It never names `describe` or the footnote arm, and no refusal a
  user can meet changes. **Samples, `CLAUDE.md`, status artifact: none**, for the reasons
  Phase 15 gave. **Letur: none.** Letur's page lists no footnote refusal, and no behaviour
  changes.
