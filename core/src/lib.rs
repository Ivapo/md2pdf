...
/// columns come out of one parse with one set of options, and `emit::options`
/// is `pub(crate)`. Every other home for this function is a second copy of it.
pub fn md_to_html(md: &str) -> String {
    let mut html = String::new();
    pulldown_cmark::html::push_html(&mut html, Parser::new_ext(md, emit::options()));
    html
}
